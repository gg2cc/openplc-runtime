...
{
    if (fd < 0 || !can_id || !payload) return -1;

    struct can_frame frame;
    ssize_t nbytes = read(fd, &frame, sizeof(struct can_frame));
    if (nbytes < 0) {
        /* Timeout or interrupt is expected */
        return -1;
    }
...
int can_socket_write(int fd, uint32_t can_id, bool eff, bool rtr, uint8_t dlc, const uint8_t *payload)
{
    ...
    frame.can_id = can_id;
    if (eff) frame.can_id |= CAN_EFF_FLAG;
...
int can_socket_read(int fd, uint32_t *can_id, bool *eff, bool *rtr, uint8_t *dlc, uint8_t *payload)
{
    (void)fd; (void)can_id; (void)eff; (void)rtr; (void)dlc; (void)payload;
    return -1;
}
