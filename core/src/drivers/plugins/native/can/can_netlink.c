...
static int send_nl_req(int fd, struct nl_req *req, plugin_logger_t *logger)
{
    ...
    struct iovec iov = {.iov_base = req, .iov_len = req->nh.nlmsg_len};
    ...
    char reply_buf[1024];
    struct iovec reply_iov  = {.iov_base = reply_buf, .iov_len = sizeof(reply_buf)};
    struct msghdr reply_msg = {
        .msg_name = &nladdr, .msg_namelen = sizeof(nladdr), .msg_iov = &reply_iov, .msg_iovlen = 1};
...
int can_netlink_configure_and_up(const can_hardware_config_t *hw, plugin_logger_t *logger)
{
    ...
    if (linkinfo)
    {
        ...
        if (infodata)
        {
            ...
            if (hw->restart_ms > 0)
            {
                ...
                add_attr(&req.nh, sizeof(req), IFLA_CAN_RESTART_MS, &restart_ms,
                         sizeof(restart_ms));
            }
    ...
    if (ret != 0)
    {
        plugin_logger_warn(logger,
                           "Failed to apply CAN bitrate/timing on %s via Netlink (may require root "
    ...
    if (hw->auto_bringup)
    {
        ...
        {
            plugin_logger_warn(logger, "Failed to bring CAN interface %s UP via Netlink",
                               hw->interface);
        }
    }
