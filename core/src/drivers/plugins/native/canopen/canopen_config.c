...
static void parse_od_entry(cJSON *item, canopen_od_entry_t *entry, plugin_logger_t *logger)
{
    ...
    if (logger) {
        plugin_logger_debug(logger,
                            "OD entry: name=%s index=0x%04X sub=%u type=%s pdo_map=%s",
...
static void parse_pdo(cJSON *items, int *count, canopen_pdo_t *pdo_list, const char *pdo_name, plugin_logger_t *logger)
{
    ...
    *count = 0;
    ...
    for (int i = 0; i < total && i < MAX_CANOPEN_PDO_COUNT; i++) {
        ...
        if (cJSON_IsArray(mapping)) {
            ...
            pdo->mapping_count = 0;
            for (int map_idx = 0; map_idx < map_total && map_idx < MAX_CANOPEN_PDO_MAPPING; map_idx++) {
        ...
        if (logger) {
            plugin_logger_debug(logger, "PDO parsed: %s index=0x%04X mappings=%d", pdo_name, pdo->index, pdo->mapping_count);
        }
        (*count)++;
    }
}
...
static void parse_bus(cJSON *item, canopen_bus_config_t *bus, plugin_logger_t *logger)
{
    ...
    if (bus->name[0] == '\0' && bus->interface[0] != '\0') {
        snprintf(bus->name, sizeof(bus->name), "%s", bus->interface);
    }

    if (logger) {
        plugin_logger_debug(logger,
                            "CANopen bus parsed: name=%s interface=%s node_id=%u bitrate=%u od_entries=%d tpdo=%d rpdo=%d",
...
int canopen_config_parse(const char *json_path, canopen_config_t *config, plugin_logger_t *logger)
{
    ...
    if (!f) {
        if (logger) plugin_logger_warn(logger, "CANopen config file not found: %s", json_path);
        return 0;
    }
    ...
    if (!buf) {
        fclose(f);
        if (logger) plugin_logger_error(logger, "Out of memory reading CANopen config");
        return -1;
    }
    ...
    free(buf);
    if (!root) {
        if (logger) plugin_logger_error(logger, "Failed to parse CANopen config JSON: %s", json_path);
        return -1;
    }
    ...
    if (!cJSON_IsArray(buses)) {
        cJSON_Delete(root);
        if (logger) plugin_logger_error(logger, "CANopen config must contain a buses[] array");
        return -1;
    }
    ...
    if (bus_total > MAX_CANOPEN_BUSES) {
        ...
        if (logger) plugin_logger_warn(logger, "Truncating CANopen buses to %d", MAX_CANOPEN_BUSES);
    }
    ...
    for (int i = 0; i < bus_total; i++) {
        ...
        if (config->buses[config->bus_count].enabled) {
            ...
            plugin_logger_debug(logger, "Skipping disabled CANopen bus: %s", config->buses[config->bus_count].name);
        }
    }
